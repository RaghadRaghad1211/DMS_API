...
namespace DMS_API.Services
{
    public class FolderService
    {
        ...
        private List<FolderModel> Folder_Mlist { get; set; }
            ...
            try
            {
                ...
                else
                {
                    ...
                    var MaxTotal = dam.FireDataTable($"SELECT COUNT(*) AS TotalRows, CEILING(COUNT(*) / CAST({_PageRows} AS FLOAT)) AS MaxPage " +
                                             $"FROM [User].V_Folders  WHERE [OrgOwner] IN ({whereField} FROM [User].[GetOrgsbyUserId]({userLoginID})) AND ObjClsId ={ClassID} ");
                    if (MaxTotal == null)
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            Folder_Mlist = new List<FolderModel>();
                            if (dt.Rows.Count > 0)
                            {
