            ...
            try
            {
                ...
                else
                {
                    ...
                    var MaxTotal = dam.FireDataTable($"SELECT COUNT(*) AS TotalRows, CEILING(COUNT(*) / CAST({_PageRows} AS FLOAT)) AS MaxPage " +
                                             $"FROM [User].V_GroupsFolders  WHERE [OrgOwner] IN (SELECT [OrgId] FROM [User].[GetOrgsbyUserId]({userLoginID})) AND ObjClsId ={ClassID} ");
                    if (MaxTotal == null)
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            string getGroupInfo = "SELECT  ObjId, ObjTitle, ObjClsId, ClsName, ObjIsActive, ObjCreationDate, ObjIsDesktopFolder, ObjDescription, UserOwnerID, " +
                                                ...
                                                $"WHERE [OrgOwner] IN (SELECT [OrgId] FROM [User].GetOrgsbyUserId({userLoginID})) AND ObjClsId ={ClassID} " +
                                                 "ORDER BY ObjId " +
            ...
            try
            {
                ...
                else
                {
                    ...
                    string getGroupInfo = "SELECT  ObjId, ObjTitle, ObjClsId, ClsName, ObjIsActive, ObjCreationDate, ObjIsDesktopFolder, ObjDescription, UserOwnerID, " +
                                                     "            OwnerFullName, OwnerUserName, OrgOwner, OrgEnName,OrgArName , OrgKuName " +
                                                     "FROM            [User].V_GroupsFolders " +
                                                    $"WHERE ObjId={id} AND [OrgOwner] IN (SELECT [OrgId] FROM [User].GetOrgsbyUserId({userLoginID})) AND ObjClsId ={ClassID} ";
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        int checkDeblicate = Convert.ToInt32(dam.FireSQL($"SELECT COUNT(*) FROM [Main].[Objects] WHERE ObjTitle = '{Group_MV.ObjTitle}' AND " +
                                                                         $"[ObjOrgOwner] IN (SELECT [OrgId] FROM [User].GetOrgsbyUserId({userLoginID})) AND ObjClsId ={ClassID} "));
                        if (checkDeblicate == 0)
                        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (checkDeblicate == 0)
                        {
                            string exeut = $"EXEC [User].[UpdateGroupOrFolderPro]'{Group_MV.ObjTitle}', '{Group_MV.ObjIsActive}','{Group_MV.ObjDescription}' ";
                            var outValue = await Task.Run(() => dam.DoQueryExecProcedure(exeut));
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        string getgroupInfo = "SELECT  ObjId, ObjTitle, ObjClsId, ClsName, ObjIsActive, ObjCreationDate, ObjIsDesktopFolder, ObjDescription, UserOwnerID, " +
                                                     "            OwnerFullName, OwnerUserName, OrgOwner, OrgEnName,OrgArName , OrgKuName " +
                                                     "FROM            [User].V_GroupsFolders " +
                                                  $"WHERE  ObjTitle LIKE '{Name}%' AND [OrgOwner] IN (SELECT [OrgId] FROM [User].GetOrgsbyUserId({userLoginID})) AND ObjClsId ={ClassID} ";
